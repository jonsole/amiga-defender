...
GFX_Init:		; Set interrupt3 vector
			...
			MOVE.W 	#(DMACON_SET|DMACON_DMAEN|DMACON_BPLEN|DMACON_SPREN|DMACON_COPEN|DMACON_BLTEN|DMACON_BLTPRI),(DMACON,A6)

			; Enable vertical blank interrupt 
			MOVE.W	#(INTENA_SET|INTENA_INTEN|INTENA_VERTB),(INTENA,A6)

			MOVE.L	#SpriteData,D1
			LEA	GFX_CopperSpriteInit,A1
			MOVEQ	#7,D0
.SpriteLoop:		MOVE.W	D1,(6,A1)
			SWAP	D1
			MOVE.W	D1,(2,A1)
			SWAP	D1
			ADD.W	#8,A1
...
*****************************************************************************
* Name
*   GFX_AllocMemAlign64K: Allocate memory within a 64K aligned block
* Synopsis
*   Memory = GFX_AllocMemAlign64K(Size)
*   D0                            D2
* Function
*   This function attempts to allocate a block of memory that's entirely in
*   a 64K block.
* Registers
*   
*****************************************************************************
			XDEF GFX_AllocMemAlign64K
GFX_AllocMemAlign64K:	MOVE.L	A6,-(SP)
			MOVE.L	4.W,A6
			; D3 = Address
			; D2 = Size
			MOVEQ #0,D3

.Loop:			; Copy address and size into registers for Exec call
			MOVE.L	D2,D0
			MOVE.L	D3,A1
			JSR	(_LVOAllocAbs,A6)

			; Exit if allocation was successful
			TST.L	D0
			BNE	.Done
...
; Table holding next available Y position for the 4 attached sprites
GFX_SpritePosY		DC.W	0,0,0,0	

; Reset GFX_SpritePosY table and pre-load A5 and D5
...
; Write CWAIT($FF,$FE) into copper list 
...
* Synopsis
*   GFX_ADD_SPRITE(Height, PosX, PosY, SpriteData, SpriteData2, PositionTable, CopperList, Table, Mask)
*                  D0      D1    D2    D3          D4           A1             A4          A5     D5
* Function
...
* Synopsis
*   CopperList = GFX_Blit16x(Custom, Height, PosX, PosY, SpriteData, SpriteData2, CopperList, Table, Mask)
*   A4                       A6      D0      D1    D2    D3          D4            A4          A5     D5
* Function
...
* Synopsis
*   GFX_InitBlit16x(Custom, ClearList, Bitplane)
*		    A6      A2	       D6	
* Function
...
*****************************************************************************
* Name
*   GFX_Blit16x: 16 pixel wide blit
* Synopsis
...
GFX_Blit16x:		LEA	(GFX_BlitTable,PC),A1
			...
			MOVE.W	(2,A1,D1.W),D6		; Set X offset
			ADD.W	(0,A1,D2.W),D6		; Add Y offset
			...
			; Copy BLTDPT in D6 to BLTCPT in D2
			MOVE.L 	D6,D2
			...
			MOVEM.L D2/D3/D4/D6,(BLTCPTH,A6) ;44 D2->BLTCPT,D3->BLTBPT,D4->BLTAPT,D6->BLTDPT
...
*****************************************************************************
* Name
*   GFX_Blit16x: 32 pixel wide blit
* Synopsis
*   Dest,ClearList = GFX_Blit32x(Custom, Height, PosX, PosY, Bitplane BlitData, BlitMask, ClearList)
*   A0   A2                      A6      D0      D1    D2    D6       D3        D4        A2
* Function
*   This function performs a blit.
* Registers
...
GFX_Blit32x:	        LEA	(GFX_BlitTable,PC),A1
			...
			; Program blitter
			MOVE.L	D1,(BLTCON0,A6)
			MOVEM.L D2/D3/D4/D6,(BLTCPTH,A6) ;44 D2->BLTCPT,D3->BLTBPT,D4->BLTAPT,D7->BLTDPT
			MOVE.W	#DMACON_BLTPRI,(DMACON,A6)
			MOVE.W	D0,(BLTSIZE,A6)
			RTS
...
*****************************************************************************
* Name
*   GFX_ClearList16x: Clear 16 pixel wide blits
* Synopsis
...
GFX_CopperSpriteInit:	CMOVE.L	SPR0PT,0
			CMOVE.L	SPR1PT,0
			CMOVE.L	SPR2PT,0
			CMOVE.L	SPR3PT,0
			CMOVE.L	SPR4PT,0
